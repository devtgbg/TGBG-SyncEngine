/**
 * One delivery, opened from the log: what Zupersync did with it, who caused it,
 * what it says changed, and the webhook exactly as it was stored.
 *
 * A server component, so the body (customer names, addresses, job details) is
 * read and rendered behind the sign-in and only for the one delivery asked for.
 */

import Link from "next/link";
import type { DeliveryDetail } from "@/lib/db";
import { describe, dubaiTime } from "@/lib/describe";
import { outcome } from "@/lib/outcome";
import { CopyButton, EscapeTo } from "./row";

export function Detail({ d, names, closeHref }: { d: DeliveryDetail; names: Record<string, string>; closeHref: string }) {
  const o = outcome(d);
    ...
    <>
      ...
      <aside className="drawer" role="dialog" aria-modal="true" aria-label="Delivery detail">
        ...
        <header className="drawer-head">
          <div>
            <p className="mono dim">{d.event ?? "no event"}</p>
            <h2>{said.headline}</h2>
            <p className="dim">
        ...
        <section>
          ...
          {person || d.by_email ? (
            <dl className="facts">
              ...
              <dt>Zuper user uid</dt><dd className="mono wrapany">{d.by_uid ?? "—"}</dd>
            </dl>
          ) : (
        ...
        <section>
          ...
          <p className="note">
            This is what the webhook says. Zupersync does not copy these values: it re-reads the record from Zuper and writes that.
          </p>
        </section>
        ...
        <section>
          ...
          <p className="note">
            The body is as stored: every value exactly as Zuper sent it, with the keys in the order the database keeps them.
            {hidden.length ? ` ${hidden.join(", ")} is hidden because it is the secret that authenticates deliveries.` : ""}
          </p>
        </section>
      </aside>
    </>

...
const KNOWN_CAUSE: { kind?: string; path: RegExp; why: string }[] = [
  ...
  { path: /^(customer|organization|job|asset|property|request_source|team|estimate|estimate\.job|recurring_job|assets\[\]\.asset|customer_organization|customer\.customer_organization)\.updated_at$/,
  ...
  { path: /(^|\.)(user|users|users\[\]|created_by|created_by_user|created_user|requested_by|approved_by_user|done_by|sold_by_user|await_approval_by)\.(created_at|updated_at|last_login_at|profile_picture)$/,
  ...
  { kind: "users", path: /^profile_picture$/,
    why: "A person's picture is Tuper's own copy of it (jms-avatars, 2026-09-14), not Zuper's S3 link. Their created_at, updated_at and last sign-in are Zuper's since Tuper 00208." },
  ...
  { path: /(^|\.)(geo_cordinates\[\]|point_coordinates|point_coordinates\.coordinates\[\])$/,
  ...
  { kind: "invoices", path: /^line_items\[\]\.(line_item_type|line_item_uid|tax|taxes|plain_text_description|total_purchase_price|is_commissionable|section_type|show_child_prices|show_section_total)$|^custom_fields\[\]\.(hide_field|read_only)$|^(total_discount|auto_charge|surcharge_amount)$/,
    why: "Zuper's invoice has gained keys over time, as its quote has, and each keeps the set it was saved with — measured on all 13 of GBG's invoices, 2026-09-22: custom-field flags on the 4 newest, section settings on one line of the newest, auto_charge on 2; older lines carry no line_item_type, uid or tax. Tuper answers the complete invoice." },
  { kind: "invoices", path: /^line_items\[\]\.(product_uid|product_ref_id)$|^estimate\.line_items\[\]\.product_uid$/,
    why: "the line names a part Zuper has since deleted (GBG's 2021 invoices sell parts Zuper deleted in 2023). Owner decision 2026-09-21: deleted parts are not imported, so Tuper's line keeps the part's name and number but no link. product_ref_id is the part record itself, which Zuper embeds and Tuper does not." },
  { kind: "invoices", path: /^(payment_url|public_url)$/,
    why: "Zuper's links to the invoice on its own customer portal. Tuper's customer-facing pages belong to the portal, which is outside this work, so Tuper answers no link rather than one not known to open." },
  { kind: "products", path: /^created_by$/,
    why: "the part was made by someone Zuper has since deleted (GBG's 'Mike', 9f709f80…, on parts from Nov 2023): deleted people are in neither Zuper's active nor its inactive staff list, so there is no one in Tuper to point at." },
  { kind: "jobs", path: /^recurring_job\.created_by$/,
    why: "Zuper answers who made a repeat as its own internal number (94483, 131208 …), not a person; Tuper answers the person's uid." },
  { kind: "jobs", path: /^assigned_to\[\]\.(assigned_at|is_accepted)$/,
    why: "Zuper's older assignments carry no assigned time and no is_accepted (50 of 97 assignments have a time); Tuper keeps a time on every assignment, the one Zuper gave or the one it was written at." },
  { path: /(^|\.)job_category\.estimated_duration\.days$/,
    why: "a category whose duration never had days set answers days: null in Zuper (39 of 89 jobs) and 0 on those that did; Tuper keeps the duration in minutes and answers 0." },
  ...
  { path: /^(job\.|parent_job\.)?(current_job_status\.(status_name|status_type)|job_status\[\]\.(status_name|status_type))$/,
  ...
  { kind: "jobs", path: /^(all_day_schedule|is_dispatchable|is_scheduled|job_appointments|total_rebate|service_task|gallery|material_request|purchase_order|waiting_on_mr|waiting_on_po|waiting_on_mr_uids|waiting_on_po_uids|job_total|discount|due_date_dt|job_timezone|current_job_status\.category)(\.|$)|^associations\.(material_requests|purchase_orders|service_orders|pending_documents|total_documents|total_service_tasks|pending_service_tasks)$|^profitability\.(revenue|calculate_job_revenue_from)|^profitability$|^parent_job\.job_timezone$/,
    why: "Zuper's job has gained keys over time and each job keeps the set it was saved with — measured on 89 jobs from 2021 to 2026, 2026-09-21: all_day_schedule only on jobs from Aug 2026, is_dispatchable/is_scheduled/gallery/associations.total_documents from Jun 2026, job_appointments from May 2026, total_rebate and service_task from Aug-Sep 2026, profitability and material_request from Aug 2025, discount from Sep 2025, job_total from Aug 2023, job_timezone from May 2024. Tuper answers the complete job for every job." },
  { path: /^(parent_job\.|job\.)?job_status\[\]\.(status_history_uid|updated_at|synced_at|is_offline)$/,
    why: "a timeline entry's bookkeeping: older entries carry no history uid, updated_at, synced_at or is_offline in Zuper (the same growth as the job's own keys), and where they do, updated_at is Zuper's clock while Tuper's is when the sync wrote the row." },
  { path: /(^|\.)role\.(created_at|updated_at)$/,
    why: "a person's role. Zuper's three system roles date from its 2018 install; Tuper's are its own rows, made when the tenant was." },
  { path: /(^|\.)(user|users\[\]|done_by|created_by|requested_by|approved_by_user|created_by_user)\.is_deleted$/,
    why: "Zuper's inactive staff were imported as removed (owner decision 2026-09-15) so no picker offers them; Zuper calls them inactive, not deleted." },
  { kind: "users", path: /^is_deleted$/,
    why: "the same: an inactive person in Zuper is a removed one in Tuper (owner decision 2026-09-15)." },

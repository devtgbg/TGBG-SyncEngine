...
const KNOWN_CAUSE: { kind?: string; path: RegExp; why: string }[] = [
  ...
  { path: /^(customer|organization|job|asset|property|request_source|customer\.customer_organization)\.updated_at$/,
  ...
  { kind: "requests", path: /^(request_status\.|status_history\[\])/,
    why: "OWNER DECISION PENDING. Tuper's request statuses are its four seeds (New, In Review, Converted, Closed — no colour); GBG's Zuper account uses Open, Booked and Canceled (+ an On Hold). Replacing Tuper's with Zuper's changes the Requests screens' workflow, so it waits on the owner." },
  ...
  { path: /^job\.custom_fields\[\]$/,
    why: "JOB RE-SYNC NEEDED (owner's OK). Zuper lists every custom field a job carries, empty ones too; the jobs importer still uses the older helper that skips empty values, so a job's empty fields are missing. Moving jobs to writeZuperCustomFields means re-syncing 46,577 jobs." },
